...
async function setupDockerProject(languages: string[], context: vscode.ExtensionContext, rebuild: boolean = false): Promise<boolean> {
  try {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
    ...
    const dockerContainerDir = vscode.Uri.joinPath(workspaceFolder.uri, "DockerContainer");
    ...
    console.log("About to generate Docker files in:", dockerContainerDir.fsPath);
    ...
    if (rebuild) {
      ...
      const repositoryName = workspaceFolder.name.toLowerCase().replace(/[^a-z0-9]/g, '');
      ...
      try {
        ...
        await execAsync(`cd "${dockerContainerDir.fsPath}" && docker build -t "${containerName}" .`);
...
async function moveExistingFilesToSrc(workspaceFolder: vscode.WorkspaceFolder): Promise<void> {
  const { exec } = require("child_process");
  const { promisify } = require("util");
  const execAsync = promisify(exec);
  ...
  try {
    ...
    console.log("Files to move to src/:", filesToMove.map(([name]) => name));
    ...
    for (const [name, type] of filesToMove) {
      try {
        ...
        await execAsync(`cd "${workspacePath}" && git mv ${sourcePath} ${destPath}`);
        ...
        try {
          ...
          const { promisify } = require("util");
          const execAsync = promisify(exec);
          await execAsync(`cd "${workspacePath}" && mv "${name}" "src/${name}"`);
          console.log(`Fallback: moved ${name} to src/ using mv`);
        } catch (fallbackError) {
          console.error(`Fallback move also failed for ${name}:`, fallbackError);
        }
      }
    }
...
async function generateDockerFiles(dockerContainerDir: vscode.Uri, languages: string[], context: vscode.ExtensionContext): Promise<void> {
  try {
    console.log("generateDockerFiles called with:", dockerContainerDir.fsPath, languages);
    const extensionPath = context.extensionPath;
    console.log("Extension path:", extensionPath);

    const setupFilesDir = vscode.Uri.joinPath(vscode.Uri.file(extensionPath), "DockerSandbox", "setup_files");
    console.log("Setup files directory:", setupFilesDir.fsPath);
    
    // Generate Dockerfile based on selected languages
    const dockerfileContent = await generateDockerfile(setupFilesDir, languages);
    const dockerfilePath = vscode.Uri.joinPath(dockerContainerDir, "Dockerfile");
    console.log("Writing Dockerfile to:", dockerfilePath.fsPath);
    await vscode.workspace.fs.writeFile(dockerfilePath, Buffer.from(dockerfileContent, 'utf-8'));
    console.log("Generated Dockerfile");

    // Generate entrypoint script based on selected languages
    const entrypointContent = await generateEntrypointScript(setupFilesDir, languages);
    const entrypointPath = vscode.Uri.joinPath(dockerContainerDir, "entrypoint2.sh");
    console.log("Writing entrypoint script to:", entrypointPath.fsPath);
    await vscode.workspace.fs.writeFile(entrypointPath, Buffer.from(entrypointContent, 'utf-8'));
    console.log("Generated entrypoint script");

    // Copy build and run scripts
    const sourceDockerDir = vscode.Uri.joinPath(vscode.Uri.file(extensionPath), "DockerSandbox", "DockerContainer");
    
    // Copy build script
    try {
      const buildScriptSource = vscode.Uri.joinPath(sourceDockerDir, "build_docker.sh");
      const buildScriptDest = vscode.Uri.joinPath(dockerContainerDir, "build_docker.sh");
      console.log("Copying build script from:", buildScriptSource.fsPath, "to:", buildScriptDest.fsPath);
      const buildScriptContent = await vscode.workspace.fs.readFile(buildScriptSource);
    ...
    // Copy run script
    try {
      const runScriptSource = vscode.Uri.joinPath(sourceDockerDir, "run_docker.sh");
      const runScriptDest = vscode.Uri.joinPath(dockerContainerDir, "run_docker.sh");
      console.log("Copying run script from:", runScriptSource.fsPath, "to:", runScriptDest.fsPath);
      const runScriptContent = await vscode.workspace.fs.readFile(runScriptSource);
...
async function generateDockerfile(setupFilesDir: vscode.Uri, languages: string[]): Promise<string> {
  let dockerContent = '';
  ...
  if (languages.includes('java')) {
    const dockerJavaPath = vscode.Uri.joinPath(setupFilesDir, "docker_java.txt");
    const dockerJavaContent = await vscode.workspace.fs.readFile(dockerJavaPath);
    dockerContent += Buffer.from(dockerJavaContent).toString('utf-8') + '\n';
  }
  
  if (languages.includes('python')) {
    const dockerPythonPath = vscode.Uri.joinPath(setupFilesDir, "docker_python.txt");
    const dockerPythonContent = await vscode.workspace.fs.readFile(dockerPythonPath);
...
async function generateEntrypointScript(setupFilesDir: vscode.Uri, languages: string[]): Promise<string> {
  let shContent = '';
  ...
  headContent = headContent.replace(
    ...
    `find /app -type f \\( ${supportedTypes.map(type => `-name "*${type}"`).join(' -o ')} \\)`
  );
...
async function getDockerStatus(context: vscode.ExtensionContext): Promise<{
  ...
  try {
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
      return { dockerEnabled: false, supportedLanguages: [], containerExists: false };
    }
    ...
    try {
      ...
      const dockerfilePath = vscode.Uri.joinPath(dockerContainerDir, "Dockerfile");
      try {
        const dockerfileContent = await vscode.workspace.fs.readFile(dockerfilePath);
      ...
      dockerEnabled = false;
      console.log("Docker is not enabled - DockerContainer directory not found");
    }
...
async function runCurrentFile(context: vscode.ExtensionContext, forceRebuild: boolean = false): Promise<boolean> {
  ...
  try {
    ...
    console.log(`Is file in workspace: ${currentFile.fsPath.startsWith(workspacePath)}`);
    ...
    if (!dockerEnabled) {
      ...
      switch (fileExtension) {
        ...
        case 'java':
          vscode.commands.executeCommand('workbench.action.terminal.sendSequence', {
            text: `javac "${currentFile.fsPath}" && java "${currentFile.fsPath.replace('.java', '')}"\n`
          });
          break;
        default:
    ...
    if (containerExists && !forceRebuild) {
      try {
        ...
        const containerInspectResult = await execAsync(`docker inspect ${containerName} --format='{{.Created}}'`);
        ...
        if (dockerfileModified > containerCreated || entrypointModified > containerCreated) {
          needsRebuild = true;
          console.log(`Docker files are newer than container, rebuilding...`);
        }
      } catch (error) {
        console.log(`Could not check file timestamps, rebuilding to be safe: ${error}`);
        needsRebuild = true;
      }
    }
    
    if (needsRebuild) {
      console.log(`Building Docker container: ${containerName}${forceRebuild ? ' (forced rebuild)' : ''}`);
      try {
        console.log(`Building Docker container in: ${dockerContainerDir.fsPath}`);
        ...
        vscode.window.showErrorMessage(`Failed to build Docker container: ${error}`);
        return false;
      }
    } else {
    ...
    try {
      ...
      vscode.window.showInformationMessage(`File executed with Docker: ${relativeFilePath}`);
    } catch (error) {
      console.error(`Error running file with Docker: ${error}`);
      vscode.window.showErrorMessage(`Failed to run file with Docker: ${error}`);
      return false;
    }
...
class PolycodeViewProvider implements vscode.WebviewViewProvider {
  ...
  sendDockerStatus(status: { dockerEnabled: boolean; supportedLanguages: string[]; containerExists: boolean }) {
    if (this.webview) {
      this.webview.postMessage({
        type: "dockerStatus",
        ...status
...
function hookMessages(
  ...
  webview.onDidReceiveMessage((msg) => {
    ...
    if (msg?.type === "dockerSetup" && msg.enabled && msg.languages) {
      ...
      if (context) {
        setupDockerProject(msg.languages, context, msg.rebuild).then((success) => {
          if (success) {
            ...
            vscode.window.showErrorMessage(`Failed to ${action} Docker project structure`);
          }
        });
      } else {
...
async function applyCRDTUpdatesToFile(updates: any[]) {
  try {
    ...
    try {
      ...
      console.log(`File ${targetUri.fsPath} doesn't exist, skipping CRDT update`);
      isApplyingCRDTUpdate = false;
      return;
    }
